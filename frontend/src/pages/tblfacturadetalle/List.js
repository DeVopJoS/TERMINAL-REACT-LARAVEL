...
const TblfacturadetalleListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "Delete",
			command: (event) => { deleteItem(data.detalle_id) },
			icon: "pi pi-trash"
		}
	]
	...
	function DetalleIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblfacturadetalle/view/${data.detalle_id}`}> { data.detalle_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblfacturadetalleListPage" className="main-page">
    { (props.showHeader) && 
...
TblfacturadetalleListPage.defaultProps = {
	primaryKey: 'detalle_id',
	pageName: 'tblfacturadetalle',
	apiPath: 'tblfacturadetalle/index',
	routeName: 'tblfacturadetallelist',
...
export default TblfacturadetalleListPage;