...
const TbledificionivelListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tbledificionivel/view/${data.nivel_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tbledificionivel/edit/${data.nivel_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.nivel_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function NivelIdTemplate(data){
		if(data){
			return (
				<Link to={`/tbledificionivel/view/${data.nivel_id}`}> { data.nivel_id }</Link>
			);
		}
	}
	...
	return (
<main id="TbledificionivelListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Edificio Nivel"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tbledificionivel/add`}>
...
TbledificionivelListPage.defaultProps = {
	primaryKey: 'nivel_id',
	pageName: 'tbledificionivel',
	apiPath: 'tbledificionivel/index',
	routeName: 'tbledificionivellist',
...
export default TbledificionivelListPage;
