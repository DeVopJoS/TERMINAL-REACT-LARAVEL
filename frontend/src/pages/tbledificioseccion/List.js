...
const TbledificioseccionListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tbledificioseccion/view/${data.seccion_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tbledificioseccion/edit/${data.seccion_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
	...
	function SeccionIdTemplate(data){
		if(data){
			return (
				<Link to={`/tbledificioseccion/view/${data.seccion_id}`}> { data.seccion_id }</Link>
			);
		}
	}
	...
	return (
<main id="TbledificioseccionListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Edificio Seccion"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tbledificioseccion/add`}>
...
TbledificioseccionListPage.defaultProps = {
	primaryKey: 'seccion_id',
	pageName: 'tbledificioseccion',
	apiPath: 'tbledificioseccion/index',
	routeName: 'tbledificioseccionlist',
...
export default TbledificioseccionListPage;
